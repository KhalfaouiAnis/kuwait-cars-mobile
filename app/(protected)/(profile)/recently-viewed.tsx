...
export default function RecentlyViewedAdsScreen() {
  ...
  const renderItem = ({ item }: { item: AdvertisementInterface }) => {
    return (
      <View className="mb-2 border border-primary-500 rounded-lg p-1 flex-row dark:bg-black">
        ...
        <View className="flex-1 flex mt-4 py-4">
          ...
          <View className="mt-1">
            <Text className="font-inter text-sm dark:text-white">
              {item.description}
            </Text>
          </View>
          <View className="flex-1 flex-row items-center justify-between px-1 mt-4">
            <Text className="text-gray-400 text-xs">
              {formatPassedTime(new Date().toDateString(), lang) ||
                "viewed 12 minutes ago"}
            </Text>
            <FavoriteButton
              isFavorite={item?.is_favorited || false}
              onPress={() => protectAction(() => mutate(item.id))}
            />
          </View>
        </View>
      </View>
  ...
  return (
    <Container header={<ProfileHeader title={t("profile.recentlyViewed")} />}>
