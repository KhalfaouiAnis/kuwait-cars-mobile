...
const Advertisement = memo(function Advertisement({ data, view = "horizontal", isDark }: Props) {
                ...
                <View className="mt-3 px-2 pb-2" style={{ direction: isRTL ? "rtl" : "ltr" }}>
                    ...
                    <View className="flex-row items-center justify-between mt-2 gap-x-0.5">
                        <View className="flex-1 flex-row items-center">
                            ...
                            <Text numberOfLines={1} className="flex-1 font-inter-medium text-base text-black dark:text-white">{t(`provinces.${data.province.province}`)}</Text>
                            <Text className="font-inter text-gray-400 ms-1">
                                {user && distanceToMyLocation(user, data)} {t(`unit.${data.mileage_unit}`)}
                            </Text>
                        </View>
                        <View className="flex-row items-center gap-x-0.5">
                            <MaterialCommunityIcons name="gas-station-outline" size={20} color={isDark ? "white" : "black"} />
                            <Text numberOfLines={1} className="font-inter text-sm text-black dark:text-white">{t(`createAd.${data.fuel_type}`)}</Text>
                        </View>
                        <View className="flex-row items-center gap-x-0.5">
                            <AntDesign name="control" size={20} color={isDark ? "white" : "black"} />
                            <Text numberOfLines={1} className="font-inter text-sm text-black dark:text-white">{t(data.transmission || "Auto")}</Text>
                        </View>
                        <FavoriteButton
    ...
    return (
        <View className="flex-row flex-1 rounded-lg border border-gray-200 bg-transparent">
            ...
            <View className="flex-1 w-full rounded-r-lg p-2 gap-y-2" style={{ direction: isRTL ? "rtl" : "ltr" }}>
                ...
                <View className="flex-row items-center justify-between">
                    <Text className="font-inter text-xs text-gray-400">{formatSmartDate(data.created_at, currentLang())}</Text>
                    <Text className="font-inter text-xs text-gray-400">{`${data.mileage} ${t(`unit.${data.mileage_unit}`)}`}</Text>
                </View>
                <View className="flex-row items-center justify-between">
                    <View className="flex-row items-center">
                        <Ionicons name="location-outline" size={14} />
                        <Text className="font-inter-medium text-sm text-black dark:text-white">{t(`provinces.${data.province.province}`)}</Text>
