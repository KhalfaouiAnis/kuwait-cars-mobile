...
export default function RadioGroup<TForm extends FieldValues>({ control, name, options, label, disabled, fullWidth, bordered, borderRadius, square }: BaseRadioInputProps<TForm>) {
    ...
    return (
        <View className='flex-1'>
            {label && <Text className="text-base font-semibold mb-2 dark:text-white text-blue">{t(label)}</Text>}
            <View
                className={cn('flex-row gap-3 flex-wrap', { "border border-grayish rounded-[20px] p-0.5 dark:border-primary-500": bordered })}
